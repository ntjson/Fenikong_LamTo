...
class StandaloneProposalTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.manager = User.objects.create_user(
            email="m@x.vn", password="pw", display_name="M"
        )
        ...
        cls.resident = User.objects.create_user(
            email="r@x.vn", password="pw", display_name="R"
        )
    ...
    def publish(self, proposal, **overrides):
        values = dict(
            ...
            purpose="Leaking roof",
            proposed_action="Replace membrane",
            expected_schedule="August 2026",
            quotation_versions=[self.quotation()],
    ...
    def test_create_publish_and_republish_platform_anchor_seven_fields(self):
        ...
        self.assertEqual(
            second.outbox_event.previous_hash, "0x" + first.outbox_event.payload_hash
        )
    ...
    def test_work_update_target_xor_is_database_enforced(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            WorkUpdate.objects.create(
                case=case, proposal=proposal, cause="x", result="y"
            )
    ...
    def test_completed_proposal_rating_requires_occupancy_and_is_once(self):
        ...
        stranger = User.objects.create_user(
            email="s@x.vn", password="pw", display_name="S"
        )
